...
class PaymentWebhookController extends Controller
{
    ...
    //Pay Pack
    public function payPack(Request $request){
        if($request->isMethod('post')){

            $currentDateTime = Carbon::now();
            $data = $request->all();
            if(isset($data['data'])){
                HiqMomo::where('payment_ref', $data['data']['ref'])->update(['payment_status' => $data['data']['status'] ]);

              if($data['data']['status'] == 'successful'){
                $momo_transaction = DB::table('hiq_momos')->where('payment_ref', $data['data']['ref'])->first();
                $transaction = DB::table('payment_transcations')->where('submit_description', $momo_transaction->payment_key)->first();
             ...
             // Update Company
                ...
                if($transaction->plan_type == 'monthly'){
                    ...
                    $update_company = DB::table('companies')
                                        ->where('id', $transaction->company_id)
                                        ->update([
                                            'status' => 'active',
                                            'subscription_plan_id' => $transaction->subscription_plan_id,
                                            'package_type' => $transaction->plan_type,
                                            'licence_expire_on' => $newDateTime,
                                       ]);

                    $transaction_update = DB::table('payment_transcations')
                                       ->where('submit_description', $momo_transaction->payment_key)
                                       ->update([
                                            'status' => 'approved',
                                            'total' => $data['data']['amount'],
                                            'paid_on' => $currentDateTime,
                                            'next_payment_date' => $newDateTime
                                       ]);
                }//Yearly
                else{
               ...
               $message = 'Well Done';
               return response()->json(['status' => True, 'message'=> $transaction]);
              }
